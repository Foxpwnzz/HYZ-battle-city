...
internal class Game
{
    ...
    private readonly double _tickSeconds = 0.5d;
    ...
    {
        _showTextState.RenderLevel(_level);

        string baseDirectory = AppDomain.CurrentDomain.BaseDirectory;
        string projectDirectory = Path.Combine(baseDirectory, "..", "..", "..");
        string levelFilePath = Path.Combine(projectDirectory, "Levels", $"level{_level}.txt");

        if (File.Exists(levelFilePath))
        {
            _levelMapManager.Clear();
            _levelMapManager.LoadLevel(levelFilePath);
        ...
        {
            _running = false;
            Console.WriteLine($"Level file not found: {levelFilePath}");
        }
    }
    ...
    public async Task RunAsync()
    {
        ...
        ShowTextState.RenderGameOverScreen(_tank.Health > 0);
